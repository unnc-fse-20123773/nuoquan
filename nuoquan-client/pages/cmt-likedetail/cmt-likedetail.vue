				...
				<swiper-item class="swiper-box" v-for="(swiperData, index1) in swiperDataList" :key="index1">
					...
					>
						...
						<view v-for="(item, index2) in index1 == 0 ? likeList : commentList" :key="index2">
							<!-- ******************** 点赞文章卡片 ********************** -->
							<view v-if="item.action == LIKEARTICLE" class="cmtlikeDetail-card" @tap="goToArticle(likeList[index2].data.target.id)">
									...
									<view class="id-line-abs">
										...
										<view class="id-line-rel">
											<view class="clTouxiang-box">
												<image
													class="clTouxiang"
													:src="pathFilter(item.data.source.faceImg)"
													mode="scaleToFill"
													@tap="goToPersonPublic(likeList[index2].data.source.userId)"
												></image>
											</view>
											<view class="clID-box">
											...
											<!-- 需要获取新消息时间戳 -->
											<view class="clID-time">{{ timeDeal(item.data.source.createDate) }}</view>
										</view>
									</view>

									<!-- 文章预览块 -->
									<view
										:class="[item.data.imgList.length > 0 ? 'origin-bar-abs-img' : 'origin-bar-abs-noimg']"
										@tap="goToArticle(likeList[index2].data.target.id)"
									>
										<view class="origin-bar-rel">
											<view class="origin-imageBox" v-if="item.data.target.imgList.length > 0">
												...
												<view class="origin-imageMasknum super_center">
													<view class="origin-imageMasknumtext">+{{ item.data.target.imgList.length }}</view>
												</view>
							...
							<!-- ************************* 点赞评论卡片 *************************** -->
							<view v-if="item.action == LIKECOMMENT" class="cmtlikeDetail-card" hover-class="hoverColor">
								<view style="width: 100%;height: 100%;" hover-class="hoverColor">
									<!-- 卡片高度未定义，上下边距会失效，用 marginHelper 填充空白 -->
									<view class="marginHelper1"></view>
									<!-- ID 行 -->
									...
									<view class="brief-bar-abs" @tap="goToComment(likeList[index2].data.target.articleId)">
										<view class="brief-bar-rel">{{ item.data.target.comment }}</view>
									</view>
							...
							<!--***************************** 评论文章卡片 *****************************-->
							<view v-if="item.action == COMMENTARTICLE" class="cmtlikeDetail-card" hover-class="hoverColor">
								<view style="width: 100%;height: 100%;" hover-class="hoverColor">
									<!-- 卡片高度未定义，上下边距会失效，用 marginHelper 填充空白 -->
									<view class="marginHelper1"></view>
									<!-- ID 行 -->
										...
										<view class="id-line-rel">
											<view class="clTouxiang-box">
												<image
													...
													:src="pathFilter(item.data.source.faceImg)"
													mode="scaleToFill"
													@tap="goToPersonPublic(commentList[index2].data.source.fromUserId)"
												></image>
											</view>
											<view class="clID-box">
												<text class="clID-text">{{ item.data.source.nickname }}</text>
							...
							<!-- ***************************** 评论评论卡片 ******************************** -->
							<view v-if="item.action == COMMENTCOMMENT" class="cmtlikeDetail-card" hover-class="hoverColor">
								<view style="width: 100%;height: 100%;" hover-class="hoverColor">
									<!-- 卡片高度未定义，上下边距会失效，用 marginHelper 填充空白 -->
									<view class="marginHelper1"></view>
									<!-- ID 行 -->
									...
									<view @tap="goToComment(commentList[index2].data.source.articleId)">
										...
										<view class="brief-bar-abs-cmtofcmt">
											<view class="brief-bar-rel-cmtofcmt">{{ item.data.target.comment }}</view>
										</view>
									</view>
									<!-- 卡片高度未定义，上下边距会失效，用 marginHelper 填充空白 -->
									<view class="marginHelper2"></view>
